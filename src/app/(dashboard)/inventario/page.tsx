...
export default function InventarioPage() {
  ...
  const [movimientosOffset, setMovimientosOffset] = useState(0);
  const [totalMovimientos, setTotalMovimientos] = useState(0);
  const [currentMovimientosPage, setCurrentMovimientosPage] = useState(1);

  useEffect(() => setCurrentPage(Math.floor(itemOffset / itemsPerPage) + 1), [itemOffset, itemsPerPage]);
  const pageCount = Math.ceil(totalCount / itemsPerPage);

  useEffect(() => setCurrentMovimientosPage(Math.floor(movimientosOffset / movimientosPerPage) + 1), [movimientosOffset, movimientosPerPage]);
  const movimientosPageCount = Math.ceil(totalMovimientos / movimientosPerPage);
  ...
  const endMovimientosOffset = movimientosOffset + movimientosPerPage - 1;
  const paginatedMovimientos = useMemo(() => {
    return movimientos.slice(movimientosOffset, endMovimientosOffset + 1);
  }, [movimientos, movimientosOffset, movimientosPerPage]);
            ...
            <div className="px-6 py-4 border-t border-gray-200">
              <div className="flex flex-col md:flex-row justify-center md:justify-between items-center gap-4 w-full p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">Página {currentMovimientosPage} de {movimientosPageCount}</span>
                  <Button variant="outline" size="sm" onClick={() => setMovimientosOffset(Math.min(movimientosOffset + movimientosPerPage, totalMovimientos - movimientosPerPage))} disabled={movimientosOffset + movimientosPerPage >= totalMovimientos}>
                    Siguiente
                  </Button>
                </div>
              </div>
