...
-- Crear RLS (Row Level Security) para paginas_legales
ALTER TABLE paginas_legales ENABLE ROW LEVEL SECURITY;

-- Política RLS: Todos pueden leer las páginas legales
CREATE POLICY "Todos pueden leer paginas legales" ON paginas_legales
    FOR SELECT USING (true);

-- Política RLS: Solo admin_global puede modificar las páginas legales
CREATE POLICY "Solo admin_global puede modificar paginas legales" ON paginas_legales
    FOR ALL USING (
        EXISTS (
